...
| 9 | `.env` set `SESSION_COOKIE_MAX_AGE_SECONDS=28800` but sessions last 7 days | Compose reads a different env name (drift); `.env` value is dead | Verify the interpolation chain: `.env` → compose `${SESSION_COOKIE_MAX_AGE_SECONDS}` → app native env (e.g. `SESSION_COOKIE_AGE`) |
...
| 11 | Active user got bounced to Cognito after `SESSION_COOKIE_MAX_AGE_SECONDS` even though they were clicking | `SESSION_COOKIE_REFRESH_SECONDS` not set or `0` — sliding refresh disabled | Set it to a value < `SESSION_COOKIE_MAX_AGE_SECONDS` (typically 1h); compose default 3600 |
...
| 14 | Logout redirects to a Cognito `/logout` URL that doesn't exist | App still emits the legacy `/oauth2/sign_out` hop | SPA navigates to the bare portal host (`window.location.hostname.replace(/^[^.]*\./, "foss.")`); no `/oauth2/sign_out` |
