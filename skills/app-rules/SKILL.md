...
- **Logout** clears app session + derives the portal host from `window.location.hostname` via `replace(/^([^-]+)-[^.]+\.(.+)/, "$1.$2")` — no env var, no build-arg, no runtime substitution. Same one-liner across every fork. Hardcoded prefixes (`"foss."`, `"moneta."`, etc.) are forbidden — they silently break when the deployment moves domains. Earlier `SMB_NAME`-env approach was retired because the multi-link plumbing chain (devstack env → fork Dockerfile / runtime entrypoint → bundle) broke silently whenever any link was missed
...
| 14 | logout shape (1-layer, no `/oauth2/sign_out`, hostname-regex portal derivation, no hardcoded prefix) | `<status>` | logout file checked; on ❌ flag any re-introduction of `/oauth2/sign_out`, a hardcoded portal prefix (`"foss."` / `"moneta."` / etc.), an `SMB_NAME` env read (retired — the regex `replace(/^([^-]+)-[^.]+\.(.+)/, "$1.$2")` is the canonical form), or any logout file diverging from that one-liner |
