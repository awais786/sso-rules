...
- Port-preservation caveat: pass `window.location.host` (NOT `window.location.hostname`) to the rewriter — `hostname` strips the port per the URL spec, breaking logout on non-standard-port deployments. Twenty's `buildPortalUrl` already does this correctly (twenty-fork commit `16f51d8`); other apps inherit the issue silently if their logout file uses `hostname`.
