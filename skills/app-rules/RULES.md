...
| Bypass paths | `/god-mode`, `/api/instances`, `/_next/static`, `/static/` | `/api/hooks`, `/_next/static` | `/js/`, `/css/`, `/images/`, `/fonts/` | `/health`, `/docs`, `/openapi.json`, `/_next/static`, `/zero` | none — SSO controller redirects unauth users server-side before assets load (see notes) |
...
**Twenty**
- Multi-workspace by design, but SSO is single-tenant: `SMB_NAME` env (the same canonical per-deployment tenant identifier every devstack app reads) drives the workspace `subdomain` + `displayName` at seed time and tells the provisioning service which workspace SSO users join. Multi-tenant SSO would require routing by Cognito attribute / email domain / host — not implemented yet.
- Workspace bootstrap is automated via `provision/provision-twenty.sh`: (1) `yarn command:prod workspace:seed:dev --light` creates the SMB_NAME workspace + standard roles (Admin, Member); the dev-seeder is gated on `SMB_NAME` to skip the Tim/Jane/Jony/Phil + ~200 random demo users. (2) `yarn command:prod workspace:bootstrap-sso-admin --email "${ADMIN_EMAIL}"` pre-seeds `ADMIN_EMAIL` (the same env Plane uses) as the workspace Admin — without it every SSO user joins as Member and hits PERMISSION_DENIED on `/metadata` mutations. Both steps idempotent on re-runs.
- No bypass router — Twenty's SSO controller redirects unauth users server-side before assets load, so all paths flow through ForwardAuth. The single `twenty-secure` router catches `/static/`, `/assets/`, `/favicon.ico`, and the SSO endpoint alike. `/healthz` is internal-only (Docker healthcheck on `localhost:3000`); Traefik never proxies it.
...
- SECURITY: `auth/sso/proxy-login` is a `@PublicEndpointGuard` route that trusts `X-Auth-Request-Email`. Trust chain (any link broken = auth-bypass): (1) Twenty's `:3000` is unpublished; (2) Traefik's `twenty-secure` runs `strip-auth-headers` BEFORE `mpass-auth`; (3) oauth2-proxy ForwardAuth re-injects the headers from the validated session. Documented inline at the controller class header. Both the `AUTH_TYPE!==SSO` branch and the missing-headers branch return opaque 404 (no diagnostic body that would tell an internal-network attacker they hit a real SSO endpoint).
- Logout regex `/^([^-]+)-[^.]+\.(.+)/ → $1.$2` (in `auth/utils/buildPortalUrl.ts`) is **tighter** than the canonical loose `/^[^.]*\./` form used by sibling apps — requires a hyphen segment so `localhost` and non-`<prefix>-<app>` hosts no-op safely.
